...
MACRO_KEYWORDS = [
    ...
    "risk", "sanction", "tariff", "volatility", "vix"
]
MACRO_STOP = [
    "ces", "humanoid", "robot", "smart glasses", "tv", "chips", "autonomous car",
    "unveils", "day 1", "show floor", "gadget"
]
...
def score_headline(title: str) -> int:
    ...
    # small penalties for very producty vibe
    if any(x in t for x in ["unveils", "launches", "details new", "day 1", "hands-on"]):
        score -= 1
...
def fetch_google_headlines(hours_back: int, query: str, debug: bool=False) -> List[Headline]:
    ...
    def pull(url: str) -> List[Headline]:
        ...
        for e in (feed.entries or [])[:100]:
            ...
            # Google News: "Headline - Source"
            source = "Google News"
            title = raw_title
            if " - " in raw_title:
                a, b = raw_title.rsplit(" - ", 1)
                title, source = a.strip(), b.strip()
            ...
            out.append(Headline(title=title, source=source, link=link, published_utc=published_utc))
        return out
    ...
    # sort by (macro score desc, recency desc)
    uniq.sort(key=lambda x: (score_headline(x.title), _parse_published_utc(x.published_utc)), reverse=True)

    # primary pick: score >= 2
    filtered = [x for x in uniq if score_headline(x.title) >= 2]

    # backfill: ensure at least 5 candidates so script doesn't get too short
    if len(filtered) < 5:
        for x in uniq:
            if x in filtered:
                continue
            # avoid very negative ones
            if score_headline(x.title) < 0:
                continue
            filtered.append(x)
            if len(filtered) >= 5:
                break

    # final safety: if still empty, take uniq
    if not filtered:
        filtered = uniq[:8]
...
def fetch_fed_events_for_date(target: date, debug: bool=False) -> List[EventItem]:
    ...
    current = None
    buffer: List[str] = []
    events: List[Tuple[str, int, List[str]]] = []  # (section, day, buffer)
    ...
    for section, day, buf in events:
        ...
        # parse time + clean labels
        t24 = None
        cleaned: List[str] = []
        for x in buf:
            if x in ignore:
                continue
            if x in {"To Be Announced"}:
                continue
            if t24 is None:
        ...
        cleaned = [x for x in cleaned if x and x not in ignore]
...
def shorten_title(s: str, max_len: int = 90) -> str:
...
def build_script(target: date, headlines: List[Headline], today_events: List[EventItem], next_events: List[EventItem]) -> str:
    ...
    if top:
        ...
        for h in top:
            lines.append(f"- {shorten_title(h.title, 88)}.")
    else:
        lines.append("Overnight was quiet on major macro headlines.")

    # Add a short "market lens" line to avoid overly short scripts
    lines.append("Quick lens into the open: watch futures, Treasury yields, and the U.S. dollar for risk appetite.")

    if today_events:
        lines.append("Today’s key events (Eastern Time):")
        for e in today_events[:4]:
            when = f"{e.time_et} ET" if e.time_et else "Time TBA"
            lines.append(f"- {when}: {shorten_title(e.title, 90)}.")
    else:
        ...
        if next_events:
            ...
            for e in next_events[:3]:
                ...
                when = f"{e.time_et} ET" if e.time_et else "Time TBA"
                lines.append(f"- {dlabel}: {when} — {shorten_title(e.title, 90)}.")

    lines.append("That’s the setup — trade safe, and I’ll see you tomorrow.")

    script = " ".join(lines)

    # If still too short, add a pacing line
    words = script.split()
    if len(words) < 120:
        script += " Expect the biggest volatility around scheduled releases, especially the 8:30 a.m. ET window."

    # Hard cap for ~1 minute
    words = script.split()
    if len(words) > 165:
...
def main():
    ...
    script = build_script(target, headlines, today_events, next_events)
